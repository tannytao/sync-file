...
public final class DirWatcher {
	...
	private void register(Path pathToMonitor, int level) throws IOException {
		...
		if (debug) {
			...
			if (prev == null) {
				log.info("-->\tregister: "+pathToMonitor+"\tregisted level="+level);
			} else {
				if (!pathToMonitor.equals(prev.path)) {
					log.info("-->\tupdate: "+prev+" -> "+pathToMonitor+"\tregisted level="+level+"\n");
				}
			}
		}
		KeyState prev = keyMap.get(key);
			...
			public void run() {
				while (true) {
					...
					for (WatchEvent<?> event : key.pollEvents()) {
						...
						//事件的信息
						if (debug)
							log.info("keyEventCount:event.kind().name(): child\n");
						...
						//刷新源目录树
						StaticUtil.refreshMTreeTop(model,jtree);
					}
	...
	private void processEvent(WatchKey key, KeyState event) {
		...
		if (event.opType != OperationType.Modify
			...
			if (event.opType != OperationType.Modify
				...
				if (debug)
					log.info("key=" + key.hashCode() + "\t" + event);
